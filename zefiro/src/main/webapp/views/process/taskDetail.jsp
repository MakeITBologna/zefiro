...
<div class="container-fluid">
	...
	<ol class="breadcrumb">
		<li><a href ng-click="gotoDocumentBreadcrumb(-1, jbDetailFormDocument)"><fmt:message key="jsp.task.label"/></a></li>
	...
	<div class="row form-group">
		<!-- Preview -->
		<div class="col-sm-8">
			<!-- ALBA TODO mettere qui la preview degli items se esistono, eventualmente con un tab -->
		...
		<div class="col-sm-4">
			...
			<form name="jbTaskFormDocument" class="form-horizontal" novalidate>
				...
				<div class="row jb-form-group" ng-repeat="p in currentTaskFormModel">
				  <label for="jbTaskFormDocument-{{p.name)}}" class="control-label col-sm-4" title="{{p.title}}">{{p.title}}</label>
				  <div class="col-sm-8">
					  
					  <div ng-if="jbWorkflowUtil.decodeType(p.dataType) == 'STRING'">
					  	<div ng-if="!readOnly" ng-class="jbValidate.getClass(jbTaskFormDocument[p.name])">
					  	    <input ng-required="p.required" ng-if="jbUtil.isEmptyObject(p.allowedValues)" id="jbDetailFormDocument-{{p-name)}}" class="form-control" title="{{p.title}}" type="text" name="{{p.name}}" ng-model="p.allowValues" />
					  	  	<select ng-required="p.required" ng-if="!jbUtil.isEmptyObject(p.allowedValues)" class="form-control" id="jbDetailFormDocument-{{p-name)}}" title="{{p.title}}" name="{{p.name}}" ng-model="documentEditing.properties[p.queryName].value">
						    	<option></option>
