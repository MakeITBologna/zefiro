<%@ include file="/include/directive.jsp" %>
  ...
  <div class="modal-header">
    <h3 class="modal-title" id="modal-title" style="color: inherit;">{{title}}
      <button class="btn btn-default pull-right" type="button" ng-click="back()">
  ...
  <div class="modal-body" id="modal-body">
    <form name="jbSearchAuthForm" class="form-horizontal" novalidate>
      <div class="row input-group" style="display: table-footer-group">
        <input class="form-control" ng-model="searchAuth" ng-minlength="2" title="searchAuth" required name="searchAuth" placeholder="<fmt:message key="js.validate.Min2Char"/>"/>
 	...
 	<label class="text-danger" ng-show="jbValidate.showMessage(jbSearchAuthForm.searchAuth)"><fmt:message key="js.validate.Min2Char"/></label>
    </form>
    <div class="row">
      <div class="col-md-6">
        <ul class="jb-select-list jb-full-area form-control" uib-dropdown-menu role="menu" aria-labelledby="single-button">
          ...
          <li ng-repeat="found in foundAuthorities" data-original-index="0" data-optgroup="1">
            <div  ng-if="authType === AUTHORITY_TYPE.PERSON">
              ...
              <span>&nbsp;({{found.id}})</span>
              <span ng-if="!(!authMany && selectedAuthorities.length>0)" ng-click="selectAuthority(found)" class="pull-right">
